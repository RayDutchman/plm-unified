...
class CadInstanceData(BaseModel):
    ...
    ty: float = 0
    tz: float = 0
    rx: Optional[float] = None  # ANGLE 模式用
...
def get_sync_status(db: Session = Depends(get_db)):
    """返回工作空间数据概览，供 CATIA 工作台显示同步状态。"""
    from app.models.part import PartMaster
    total = (
        db.query(PartMaster)
        .filter(PartMaster.deleted_at.is_(None))
        .count()
    )
    return {"parts": total, "assemblies": 0, "documents": 0, "bom_items": 0, "ecrs": 0, "ecos": 0, "config_items": 0}
