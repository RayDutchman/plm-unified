...
## 代码规范约定（遵循 `docs/collaboration/milestones.md` 第二节）

- **分支与 PR**：从当前集成分支切出 `feat/dashboard-redesign`，不直接在 `dev`/`main`/集成分支上提交功能代码；完成后走 PR 流程合并。
- **命名**：TS 变量/函数 `camelCase`，组件 `PascalCase`；Python 变量/函数 `snake_case`、类 `PascalCase`、常量 `UPPER_SNAKE_CASE`。
- **注释**：一律中文，说明"做什么/为什么"，不复述"怎么做"；复杂业务判定（如"待我审批"命中逻辑）须有中文注释。
- **提交信息**：Conventional Commits `<类型>(<范围>): <描述>`，范围按分层——前端改动用 `feat(frontend)`，后端改动用 `feat(backend)`，文档用 `docs(dashboard)`。本计划各任务的提交示例已按此约定给出。
...
git commit -m "feat(frontend): 问候与相对时间纯函数"
```
...
```bash
git add frontend/src/pages/Dashboard/lib/aggregate.ts frontend/src/pages/Dashboard/lib/aggregate.test.ts
git commit -m "feat(frontend): 状态分布与逾期天数纯函数"
```
...
```bash
git add frontend/src/pages/Dashboard/lib/aggregate.ts frontend/src/pages/Dashboard/lib/aggregate.test.ts
git commit -m "feat(frontend): 最近编辑去重、收藏展平、动态格式化纯函数"
```
...
git commit -m "feat(frontend): 数据获取 hooks（最近编辑/收藏/动态流）"
```
...
git commit -m "feat(frontend): 展示型磁贴组件（问候/KPI/状态分布/最近/收藏/动态流）"
```
...
git commit -m "feat(frontend): 待我处理/我的任务磁贴外壳"
```
...
git commit -m "feat(frontend): 两区错落磁贴布局组装，替换旧仪表盘"
```
...
git commit -m "feat(backend): get_my_todos 聚合 ECR/ECO 待办（crud + 测试）"
```
...
git commit -m "feat(backend): 新增 GET /dashboard/my-todos 路由"
```
...
git commit -m "feat(frontend): 待我处理磁贴接入 my-todos 接口"
```
...
git commit -m "feat(backend): get_my_tasks 聚合我的未完成任务（crud + 测试）"
```
...
git commit -m "feat(backend): 新增 GET /projects/my-tasks 路由"
```
...
git commit -m "feat(frontend): 我的任务磁贴接入 my-tasks 接口"
```
...
```bash
git add frontend/src/pages/Dashboard/index.tsx
git commit -m "feat(frontend): 变更进行中 KPI 接入真实计数"
```
