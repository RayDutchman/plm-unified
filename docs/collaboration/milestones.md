...
| 3.6 | 在零件详情页挂 3D 预览入口：单零件走 STPViewer Modal，装配体走 `/viewer?part=X&version=A` 前端路由（P5.3 调整：以路由链接代替 iframe，共享主应用认证状态） | B | → 3.4 | ⬜ |
...
| | 5.2 零件详情页挂入口 | `PartMasters.tsx` | 详情页加 `[📦 3D装配预览]` 按钮，点击 `navigate('/viewer?part=...&version=...')` | 5.1 |
