...
| # | 行动项 | 负责 | 串/并 | 状态 |
|---|---|---|---|---|
| 1.1 | 设计新数据库 DDL（`part_masters`、`part_revisions`、`part_iterations`、`part_usage_links`、`cad_instances`、`geometries`、`binary_resources` 核心表） | **A 主写** | 串行起点 | ✅（见 data-model.md） |
...
>
> ⚠️ **待协调（1.1 × 1.4 重叠）**：A 在 `feat/m1-ddl`（@167dbae `feat(m1): add core database DDL (M1.1)`）提交了独立的 DDL；B 的 1.4 在 `dev_myPDM` 已用 **ORM 模型 + Alembic 迁移**实现同一套 9 表 schema。两者覆盖相同的 schema，需 A/B 当面对齐：以哪一份为准、A 的 DDL 是否并入 B 的迁移、字段是否完全一致。**合并前务必 reconcile，避免双重建库脚本。**
>
> 进度小结（截至本次更新）：B 侧 1.2/1.4 完成并合并 `dev_myPDM`；待 A 完成 1.3 业务语义 review 后，B 启动 1.5（JWT 认证），A 启动 1.6/1.7。
